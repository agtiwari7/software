...
class Receipt():
    ...
    def create_receipt(self):
        ...
        mid_a5_width = page_width / 2  # Midpoint to divide bottom A5 into two A6

        # Draw the horizontal dash line to divide the A4 page into two A5 sections
        c.setDash(6, 3)
        c.setLineWidth(1)
        c.line(0, mid_height, page_width, mid_height)  # Horizontal dashed line

        # Draw the vertical dash line to divide the bottom A5 section into two A6 sections
        c.line(mid_a5_width, 0, mid_a5_width, mid_height)  # Vertical dashed line

        c.setDash([])  # Reset dash for normal lines

        # Function to draw the receipt in the bottom A5 section (split into two A6 pages)
        def draw_receipt_section(x_start, y_start):
            ...
            # Father's Name
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 37, "Father Name:")
            c.setFont("Helvetica", 11)
            c.drawString(x_start + margin + 95, image_box_y - 37, f"Mr. {self.father_name}")
            # Contact and Seat
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 57, "Contact:")
            c.setFont("Helvetica", 11)
            c.drawString(x_start + margin + 95, image_box_y - 57, self.contact)
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 195, image_box_y - 57, "Seat:")
            c.setFont("Helvetica", 11)
            c.drawString(x_start + margin + 225, image_box_y - 57, self.seat)
            # Timing
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 77, "Timing:")
            c.setFont("Helvetica", 11)
            c.drawString(x_start + margin + 95, image_box_y - 77, self.timing)
            # Address
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 97, "Address:")
            c.setFont("Helvetica", 11)
            c.drawString(x_start + margin + 95, image_box_y - 97, self.address)
            # Duration
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 117, "Duration:")
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 95, image_box_y - 117, self.duration)
            # Amount in Words
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 15, image_box_y - 137, "Rupees:")
            c.setFont("Helvetica", 9)
            c.drawString(x_start + margin + 95, image_box_y - 137, self.rupees)
            ...
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x_start + margin + 150, image_box_y - 175, "Auth. Signature")
            # Draw "This is system generated slip" at the bottom
            c.setFont("Helvetica", 9)
            c.drawString(x_start + margin + 60, rect_y - 13, "This is a software generated slip.")

        # Draw receipts in bottom two sections
        draw_receipt_section(0, 0)  # Bottom-left A6 section
        draw_receipt_section(page_width / 2, 0)  # Bottom-right A6 section
